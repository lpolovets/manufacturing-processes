---
number: 138
name: "Friction welding"
part: 3
group: "Welding & Joining"
materials: [metal]
volumes: [med, high]
tooling: mod
---

## Description

Relative motion generates interface heat, after which pressure forges the parts together without bulk melting.

## Strengths and weaknesses

Creates strong repeatable joints and can join some dissimilar metals. Parts and fixtures must support the required motion and axial force.

## When to use

Use rotary friction welding for axisymmetric joints at medium-to-high volume: axles, drill pipe, engine valves, and bimetallic transitions like aluminum-to-steel or copper-to-aluminum that fusion welding cannot make. It runs like a machine-tool process and gives forged-quality joints with no filler, gas, or spatter. Parts have to tolerate high axial force and one side has to spin, which rules out thin sheet and non-round geometry. If the joint is not round, linear friction welding handles it at aerospace-level cost. If you need long seams, use friction-stir welding.

## Examples

Drill pipe, axles, cutting tools, valves.

## Videos

- https://www.youtube.com/watch?v=zhfjev_2c5I — FRICTION WELDING | How friction welding process works | Types of friction… (ADTW Study, 5 minutes, 50k+ views)
- https://www.youtube.com/watch?v=7idvmASQ4f0 — An overview of friction welding (James Sword Engineering, 6 minutes, 10k+ views)
- https://www.youtube.com/watch?v=zyxiNBCYOyo — MTI Friction Welding 101 – What is it and When should it be used (Manufacturing Technology, Inc. (MTI), 3 minutes, 5k+ views)

## Further reading

[Types of Friction Welding (MTI)](https://www.mtiwelding.com/technologies/types-of-friction-welding/) · [What Is Friction Welding? (TWI)](https://www.twi-global.com/technical-knowledge/faqs/faq-what-is-friction-welding)
