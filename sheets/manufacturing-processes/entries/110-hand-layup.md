---
number: 110
name: "Hand layup"
part: 2
group: "Composite Manufacturing"
materials: [composite]
volumes: [proto, low]
tooling: min
---

## Description

Fiber reinforcement is manually placed in a mold and impregnated with resin.

## Strengths and weaknesses

Tooling is inexpensive and very large parts are possible. Labor, voids, repeatability, resin content, and worker exposure are disadvantages.

## When to use

Choose hand layup for one-offs through a few hundred parts a year, especially large ones like hulls, tanks, and architectural shapes. It makes sense when a mold costing a few thousand dollars matters more to you than laminate quality, and when 30–40% fiber content is acceptable. If void content, repeatability, or styrene exposure are controlled requirements, use vacuum infusion instead; it gives a better laminate from the same single-sided tool. Once volumes reach the thousands, RTM or SMC take over.

## Examples

Boat hulls, covers, tanks, prototype panels.

## Videos

- https://www.youtube.com/watch?v=Evz9QUwNtn4 — Fiberglass Layup Demonstration (R. Dean Odell, 10 minutes, 10k+ views)
- https://www.youtube.com/watch?v=3SowLJBLCZo — Hand Layup (USQ Centre for Future Materials, 2 minutes, 10k+ views)
- https://www.youtube.com/watch?v=aAsOM4ic3tI — Composite manufacturing by Hand lay up method (Engr.JOY's Academy, 5 minutes, 10k+ views)

## Further reading

[How to Hand Laminate and Vacuum Bag a Part (Easy Composites)](https://www.easycomposites.eu/learning/hand-laminate-vacuum-bag-simple-part) · [Open Molding Processes Primer (ACMA Discover Composites)](https://acmanet.org/discover-composites/processes/openmolding/)
