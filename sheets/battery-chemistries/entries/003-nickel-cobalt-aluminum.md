---
number: 3
name: Nickel Cobalt Aluminum (NCA)
part: 1
group: Layered-oxide cathodes
applications: [ev]
energy: vhigh
cycles: med
cost: low
maturity: mass
---

## Description
A nickel-rich layered cathode (~LiNi0.8Co0.15Al0.05O2 and beyond) in which aluminum doping stabilizes the lattice instead of manganese. Commercialized by Panasonic and Tesla in cylindrical cells, NCA runs among the highest energy densities of any mass-produced chemistry and pairs naturally with silicon-blended graphite anodes in 18650, 2170, and 4680 formats.

## Strengths and weaknesses
Class-leading gravimetric energy and strong rate capability; a two-decade production record in Tesla vehicles. Weaknesses: the same nickel-rich sensitivities as NMC 811 — thermal stability, moisture control, gas evolution — plus a discharge curve and degradation behavior that demand sophisticated BMS work. Cycle life is adequate for vehicles but below LFP by a wide margin.

## When to use
Pick NCA when you want maximum gravimetric energy in cylindrical formats with a production record behind it — long-range EVs and high-energy e-mobility built around 2170/4680-class cells — and you have the BMS sophistication its discharge and degradation behavior demand. In practice the choice versus high-nickel NMC comes down to supplier relationships rather than chemistry; treat them as interchangeable and negotiate accordingly. Avoid it for daily-deep-cycled or cost-floor applications, where LFP's cycle life and $/kWh win by a wide margin.

## Examples
Tesla/Panasonic 2170 and 4680 cells (Model 3/Y long-range variants); high-energy cylindrical cells for e-mobility and aerospace prototypes.

## Economic profile
Cost profile similar to nickel-rich NMC: metal-dominated, with nickel the swing input. Historically tied to a small number of cell-maker relationships (Panasonic-Tesla), so supply is less commoditized than NMC. Competes head-on with 811/9-series NMC; the distinction increasingly blurs into generic "high-nickel."

## Further reading

[Challenges and Approaches of Single-Crystal Ni-Rich Layered Cathodes (National Science Review)](https://pmc.ncbi.nlm.nih.gov/articles/PMC10628913/) · [A Reflection on Lithium-Ion Battery Cathode Chemistry (Nature Communications)](https://www.nature.com/articles/s41467-020-15355-0)
