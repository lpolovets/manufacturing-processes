...
- `sheets/<slug>/` — one directory per reference sheet:
  ...
  - `h2h.html` — the "Head-to-head" tab: one comparison table of the 5–8 high-level options that actually compete, with a "Pick it when" final column (last `<td>` renders muted; first `<td>` is bold/nowrap so keep row names short). Optional: if the file is absent the tab is omitted (tab label via `h2hTab` in sheet.json, default "Head-to-head"). Uses the same shared classes as guide.html — the view reuses `class="guide"` styling.
  - `images/NNN.jpg` (optional) — per-entry illustration keyed by zero-padded entry number (.jpg/.png/.webp); rendered at the top of the card body on the website flavor only (the artifact blocks external requests). robot-end-effectors' set was AI-generated via `build/generate-images.js` (Gemini; key read from `../.gemini_api_key` OUTSIDE the repo — never commit keys; images reviewed by hand before adoption).
...
- **Further reading**: 1–2 links per entry in a `## Further reading` section (last extra section), one line, links `[Title (Publisher)](url)` separated by " · ". Policy: canonical sources only — institutional reports, trade-association primers, vendor *educational* content, open-access reviews; one practical + one deep link where both exist; **every URL must be fetch-verified live before inclusion**; skip the section rather than pad (deliberately empty as of 2026-07-26: manufacturing 005 tube drawing, 025 slush casting, 070 planing/shaping, 131 rubber calendering). Known bot-blockers that still work in browsers: ASM Digital Library, Edmund Optics, COBOD, Panasonic PDFs. Sites that block fetchers entirely (avoid): ScienceDirect, Wiley, MDPI direct (use PMC mirrors), ACS, RSC, NRC, IAEA PDFs, SemiEngineering, NDE-Ed.org, automate.org, Festo.
...
- **Landing page**: sheets group under `category` headings from sheet.json (order = first appearance across the alphabetized sheet list); cards show `question` (italic accent line), `blurb`, and entry count. A global search box (shared/landing.js, inlined at build) lazily fetches every sheet's `data.json` on first focus and searches names/groups/parts/card text across all sheets, linking to `<slug>/#<anchor>`.
